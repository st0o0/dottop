...
{
    private readonly HashSet<string> _pinnedProcesses = new();
    private readonly HashSet<string> _pinnedAdapters = new();
    private readonly HashSet<string> _pinnedContainers = new();
    ...
    public void ToggleProcessPin(int pid)
    {
        ...
        if (!_pinnedProcesses.Remove(key)) _pinnedProcesses.Add(key);
    }
    ...
    {
        if (!_pinnedAdapters.Remove(name)) _pinnedAdapters.Add(name);
    }

    public bool IsContainerPinned(string id) => _pinnedContainers.Contains(id);
    public void ToggleContainerPin(string id)
    {
        if (!_pinnedContainers.Remove(id)) _pinnedContainers.Add(id);
    }
    public int PinnedContainerCount => _pinnedContainers.Count;

    public IReadOnlyList<T> SortWithPinnedFirst<T>(IEnumerable<T> items, Func<T, bool> isPinned)
    {
        var pinned = new List<T>();
        var rest = new List<T>();
        foreach (var item in items)
        {
            if (isPinned(item)) pinned.Add(item);
            else rest.Add(item);
        }
